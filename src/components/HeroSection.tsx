...
const HeroSection = () => {
  ...
  // A robust scroll helper that accounts for layout shifts caused by Framer Motion.
  // It smoothly scrolls and continuously checks if the target moved.
  const handleDynamicScroll = (e: React.MouseEvent<HTMLAnchorElement>, targetId: string) => {
    e.preventDefault();
    const target = document.getElementById(targetId);
    if (!target) return;

    // First attempt to jump close
    target.scrollIntoView({ behavior: "smooth" });

    // Since Framer Motion reveals sections and expands the page dynamically,
    // the target might move further down during the scroll. 
    // We check its position a few times and correct it if needed.
    let checks = 0;
    const interval = setInterval(() => {
      const rect = target.getBoundingClientRect();
      // If it's not perfectly aligned with the navbar (80px), adjust it
    ...
    <section id="home" className="min-h-screen flex items-center justify-center px-6 pt-20 relative">
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          <a
            href="#projects"
            onClick={(e) => handleDynamicScroll(e, "projects")}
          ...
          <a
            href="#contact"
            onClick={(e) => handleDynamicScroll(e, "contact")}
